...
public abstract class VideoStream extends MediaStream {
	...
	protected StreamingSurfaceView mStreamingSurfaceView = null;
	 ...
	 */
	public synchronized void setSurfaceView(StreamingSurfaceView view) {
		mStreamingSurfaceView = view;
		if (mSurfaceHolderCallback != null && mStreamingSurfaceView != null && mStreamingSurfaceView.getHolder() != null) {
			mStreamingSurfaceView.getHolder().removeCallback(mSurfaceHolderCallback);
		}
		if (mStreamingSurfaceView.getHolder() != null) {
			...
			mStreamingSurfaceView.getHolder().addCallback(mSurfaceHolderCallback);
			mSurfaceReady = true;
		}
	}
	...
	public synchronized void stop() {
		if (mCamera != null) {
			...
			if (mMode == MODE_MEDIACODEC_API_2) {
				(mStreamingSurfaceView).removeMediaCodecSurface();
			}
			super.stop();
	...
	protected void encodeWithMediaRecorder() throws IOException {
		...
		try {
			...
			mMediaRecorder.setPreviewDisplay(mStreamingSurfaceView.getHolder().getSurface());
		...
		if( Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ){
			...
			try{
				ins = new ParcelFileDescriptor.AutoCloseInputStream(parcelRead);
			}
			catch (Exception e){}
			mPacketizer.setInputStream(ins);
		}else {
	...
	protected void encodeWithMediaCodecMethod2() throws RuntimeException, IOException {
		...
		(mStreamingSurfaceView).addMediaCodecSurface(surface);
		mMediaCodec.start();
	...
	protected synchronized void createCamera() throws RuntimeException {
		...
		if (mStreamingSurfaceView.getHolder() == null || !mSurfaceReady)
			throw new InvalidSurfaceException("Invalid surface !");
		...
		if (mCamera == null) {
			...
			try {
				...
				try {
					if (mMode == MODE_MEDIACODEC_API_2) {
						mStreamingSurfaceView.startGLThread();
						mCamera.setPreviewTexture(mStreamingSurfaceView.getSurfaceTexture());
					} else {
						mCamera.setPreviewDisplay(mStreamingSurfaceView.getHolder());
					}
				} catch (IOException e) {
					throw new InvalidSurfaceException("Invalid surface! : " + e.getMessage());
				}
