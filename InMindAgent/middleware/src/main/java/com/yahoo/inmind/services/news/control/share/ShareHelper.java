...
public class ShareHelper {
    ...
    {
        try {
            ReaderController.get().getPackageManager().getApplicationInfo(appName, 0);
            return true;
    ...
    private void getComponentNames(){
    	PackageManager pm = ReaderController.get().getPackageManager();
    ...
    public enum Type{
    	Facebook, Twitter, Tumblr, More, Like, Dislike
    };
    
    public void share(Type type, Context ctx/*For launching another activity*/, 
    		String title, String summary, String link, String uuid, int index)
    {
    	String pkgName = null;
    	...
    	switch(type)
    	{
            ...
            case Dislike:
                NewsArticleVector.getInstance().get(index).setDislike(true);
                return;
            case Like:
                NewsArticleVector.getInstance().get(index).setLike(true);
                return;
    		...
    		default:
    			...
    			ctx.startActivity(Intent.createChooser(sendIntent, ReaderController.get().getResources().getText(R.string.news_send_to)));
    			return;
    	}
