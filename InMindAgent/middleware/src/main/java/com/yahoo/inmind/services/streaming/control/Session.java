 ...
 * (audio samplingrate, video resolution) and then {@link Session#getSessionDescription(String destination)}.<br />
 * 
...
public class Session {
    ...
    public void removeDestination(String destination) {
        if( mDestination != null && mDestination.contains(destination) ){
            mDestination.remove( destination );
        }
    }
	...
	public void setDestination(String destination) {
        if( mDestination == null ){
            mDestination = new ArrayList<>();
        }
        if( !mDestination.contains( destination ) ){
	 ...
	 */
	public void setSurfaceView(final StreamingSurfaceView view) {
	 ...
	 */
	public String getSessionDescription( String destination ) {
	...
	public void syncStart(int id) 			
		...
		Stream stream = id == 0 ? mAudioStream : mVideoStream;
        for( String dest : mDestination ) {
            if (stream != null && !stream.isStreaming()) {
                try {
                    InetAddress destination = InetAddress.getByName(Util.extractIpAddress( dest )[0] );
