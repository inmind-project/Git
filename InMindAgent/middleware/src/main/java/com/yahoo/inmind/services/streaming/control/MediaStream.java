...
public abstract class MediaStream implements Stream {
	...
	static {
		// We determine wehther or not the MediaCodec API should be used
		try {
	...
	protected void createSockets() throws IOException {
		...
		//ojrl: check this: http://stackoverflow.com/questions/26990816/mediarecorder-issue-on-android-lollipop
		if( Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ){
			...
			final String LOCAL_ADDR = getClass().getPackage().getName() + "-";
			for (int i=0;i<10;i++) {
				try {
