...
size_t WriteFileOrCharArr(const char* st, int bytes, void* writeTo, bool isFile, int offset)
{
	size_t ret = 1;
	if (isFile)
...
void ReduceAmplitude(char* buffer, int length, double reduceFactor)
{
	int16_t* sample = (int16_t*)buffer;
	for (int i = 0; i < length / (sizeof(int16_t) / sizeof(char)); i++)
		sample[i] = (int16_t)(sample[i] * reduceFactor);
}